...
# We will use this is complete agent if required in future
    '''is_complete_agent = lambda_.Function(
                                            self, "id_bedrock_agent_creation_is_complete",
                                            function_name="agent-resource-is-complete-lambda",
                                            code=lambda_.Code.from_asset(
                                               "lambda/create-bedrock-agent-lambda"),
                                            handler="bedrock_agent_creation_lambda.is_complete",
                                            runtime=lambda_.Runtime.PYTHON_3_11,
                                            timeout = Duration.minutes(2),
                                            environment= {'AgentName': agent_name_param.value_as_string}
                                                                        )
    
            is_complete_agent.role.attach_inline_policy(
                iam.Policy(
                    self, "id_is_complete_bedrock_agent_lambda_policy",
                    policy_name = "is_complete_create_agent_lambda_policy",
                    statements = [
                        iam.PolicyStatement(
                            actions = [
                                "bedrock:*"
    ...
    is_complete_action_agent = lambda_.Function(
                                                self, "id_bedrock_agent_action_creation_is_complete",
                                                function_name="agent-action-is-complete-lambda",
                                                code=lambda_.Code.from_asset(
                                                   "lambda/create-bedrock-agent-action-lambda"),
                                                handler="bedrockactionagent.is_complete",
                                                runtime=lambda_.Runtime.PYTHON_3_9,
                                                timeout = Duration.minutes(2),
                                                #environment= {'AgentName': agent_name_param.value_as_string}
                                                                        )
            ...
            is_complete_action_agent.role.attach_inline_policy(
                iam.Policy(
                    self, "id_is_complete_bedrock_agent_action_lambda_policy",
                    policy_name = "is_complete_bedrock_agent_action_policy",
                    statements = [
                        iam.PolicyStatement(
                            actions = [
                                "bedrock:*"
